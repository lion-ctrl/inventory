...
function ProductCard({ product, onClick, bsRate, catMap }: {
  product: Product;
...
type ProductFormValues = Omit<ProductFormState, 'price' | 'stock' | 'minStock'> & {
  price: number;
  stock: number;
  minStock: number;
};
...
function ProductForm({ initial, categories, onSave, onCancel }: {
  ...
  const set = (k: keyof ProductFormState) => (e: any) => setForm({ ...form, [k]: e.target.value });
  const setNum = (k: keyof ProductFormState, allowDecimal?: boolean) => (e: any) => {
  ...
  return (
    <div className="prod-form">
      ...
      <label className="client-field">
        <span>Categoría</span>
        <select className="input cat-select" value={form.categoryId} onChange={set('categoryId')}>
          {categories.map((c) =>
...
function ProductDetailSheet({ product, onClose, onEdit, onAdjustStock, onDelete, bsRate, catMap }: {
  product: Product;
  onClose: () => void;
        ...
        <div className="prod-detail-adjust">
          ...
          <div className="prod-detail-adjust-row">
            ...
            <Button
              ...
              onClick={() => {onAdjustStock(product._id, newStock!);setAdjust('');}}>
              Guardar
            </Button>
          </div>
...
function CategoriesSheet({ categories, onClose }: {
  categories: CategoryWithCount[];
  onClose: () => void;
}) {
            ...
            message="Esta categoría tiene productos. Reasígnalos a otra categoría antes de eliminarla." />
                <label className="client-field" style={{ marginTop: 12 }}>
                  ...
                  <select className="input cat-select" value={reassignTo} onChange={(e) => setReassignTo(e.target.value)}>
                    {targets.map((c) => <option key={c._id} value={c._id}>{c.label}</option>)}
                  </select>
                </label>
              </> :
        ...
        <div className="cat-item" key={cat._id}>
          ...
          <>
                <div className="cat-item-info">
                  ...
                  <div className="cat-item-count" style={{ textAlign: "left" }}>{cat.count} producto{cat.count === 1 ? '' : 's'}</div>
                </div>
                <div className="cat-item-actions">
                  <IconButton icon="edit-3" ariaLabel="Renombrar" onClick={() => {setEditingId(cat._id);setDraft(cat.label);}} />
...
export default function ProductsScreen() {
  ...
  const openEdit = (p: Product) => {setDetail(null);setEditing(p);setEditorOpen(true);};
    ...
    try {
      if (editing) {
        ...
        if (form.sellable === false) {
          await setSellableMut({ actorId: user!._id, productId: id, sellable: false });
        }
      }
      setEditorOpen(false);
      setEditing(null);
  ...
  const adjustStock = async (id: Id<'products'>, newStock: number) => {
    try {
      await adjustStockMut({ actorId: user!._id, productId: id, stock: newStock });
    ...
    <>
      ...
      <div className="content prod-content">
        ...
        <div className="prod-grid">
            {visible.map((p) =>
          <ProductCard key={p._id} product={p} onClick={setDetail} bsRate={bsRate} catMap={catMap} />
          )}
          </div>
        }
