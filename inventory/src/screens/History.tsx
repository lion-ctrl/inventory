...
import { AppBar, Banner, Button, Chip, Icon, IconButton, Input, Sheet } from '@/components';
...
const METHOD_LABELS: Record<string, string> = { cash: 'Efectivo dólar', cash_bs: 'Efectivo Bs', card: 'Tarjeta', transfer: 'Transferencia', mobile: 'Pago móvil', zelle: 'Zelle' };
...
export function DateInputDMY({ value, min, max, onChange, placeholder = 'dd/mm/aaaa' }: DateInputDMYProps) {
  const [text, setText] = useState(value ? fmtDMY(value) : '');
  useEffect(() => {setText(value ? fmtDMY(value) : '');}, [value]);
  ...
  return (
    <div className={`hist-row ${isRefunded ? 'is-refunded' : ''}`} onClick={() => onClick(sale)}>
      <div className="hist-col-id">
        <div className="hist-cell-id mono">N° {sale.invoiceNumber}{isRefunded && <span className="hist-refund-tag">Reembolsada</span>}</div>
        <div className="hist-cell-time">{formatDateTime(sale.soldAt)}</div>
        {isRefunded && sale.refund &&
        <div className="hist-cell-refund">Reemb. {formatDateTime(sale.refund.date)}</div>}
      </div>
  ...
  return (
    <div className={`stored-card hist-card ${isRefunded ? 'is-refunded' : ''}`} onClick={() => onClick(sale)}>
      ...
      <div className="hist-card-dates">
        ...
        {sale.refund && <span>Reemb.: {formatDateTime(sale.refund.date)}</span>}
      </div>}
...
export default function HistoryScreen() {
  ...
  filter((s) => {
    const term = norm(q);
    if (!term) return true;
    return norm(s.invoiceNumber).includes(term) || norm(s.client?.name).includes(term) || norm(s.client?.taxId).includes(term);
  });

  const sorted = [...filtered].sort((a, b) => {
    switch (sort) {
      case 'date-desc':return b.soldAt - a.soldAt;
      case 'date-asc':return a.soldAt - b.soldAt;
      ...
      <div className="content hist-content">
        ...
        <div className="hist-daterange-inline">
          <label className="catalog-filter">
            <span>Desde</span>
            <DateField value={fromDate} max={toDate || TODAY_ISO} onChange={(v) => {setFromDate(v);setRange('custom');resetPage();}} />
          </label>
          <label className="catalog-filter">
            <span>Hasta</span>
            <DateField value={toDate} min={fromDate || undefined} max={TODAY_ISO} onChange={(v) => {setToDate(v);setRange('custom');resetPage();}} />
          </label>
        </div>
        ...
        <div className="hist-table">
            ...
            {visible.map((s) => <SaleTableRow key={s._id} sale={s} onClick={setDetail} bsRate={bsRate} />)}
          </div>
        }
        {sorted.length > 0 &&
        <div className="stored-grid hist-cards">
            {visible.map((s) => <SaleRow key={s._id} sale={s} onClick={setDetail} bsRate={bsRate} />)}
          </div>
        }
