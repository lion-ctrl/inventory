...
export default function ProfileScreen() {
  ...
  const grantedPerms = me.permissions === 'all'
    ? PERMISSIONS
    : PERMISSIONS.filter((p) => me.permissions && (me.permissions as Record<string, boolean>)[p.id]);

  const save = async (form: ProfileForm) => {
    try {
      await updateSelf({
        actorId: me._id,
        patch: { name: form.name, email: form.email, phone: form.phone, pin: form.pin },
      });
      setEditOpen(false);
    } catch (e: any) {
