...
import { AppBar, Banner, Button, Chip, ConfirmDialog, Icon, IconButton, Input, Sheet } from '@/components';
...
function PermToggleRow({ perm, value, disabled, onChange }: {
  perm: PermissionInfo;
  value: boolean;
  disabled?: boolean;
  ...
  return (
    <div className={`perm-row ${disabled ? 'is-disabled' : ''}`}>
      <span className="perm-row-icon"><Icon name={perm.icon} size={18} /></span>
...
function EmployeeForm({ initial, onSave, onDelete, onCancel }: {
  ...
  const [form, setForm] = useState<EmployeeFormValues>(() => ({
    ...
    { ...initial.permissions } :
    (Object.fromEntries(PERMISSIONS.map((p) => [p.id, false])) as PermissionMap)
  }));

  const update = (k: 'name' | 'email') => (e: ChangeEvent<HTMLInputElement>) => setForm({ ...form, [k]: e.target.value });
  const setPin = (e: ChangeEvent<HTMLInputElement>) => setForm({ ...form, pin: e.target.value.replace(/\D/g, '').slice(0, 6) });
  const togglePerm = (id: PermissionInfo['id'], v: boolean) => setForm((f) => ({ ...f, permissions: { ...f.permissions, [id]: v } }));
  ...
  const dirty = !initial ? true : (
    ...
    PERMISSIONS.some((p) => !!form.permissions[p.id] !== !!initialPerms[p.id])
  );
  ...
  return (
    <div className="client-form emp-form">
      <label className="client-field">
        ...
        <Input value={form.name} onChange={update('name')} placeholder="Nombre del empleado" />
      </label>
      ...
      <label className="client-field">
        ...
        <Input type="email" value={form.email} onChange={update('email')} placeholder="empleado@correo.com" />
      </label>
      <label className="client-field">
        <span>Teléfono</span>
      ...
      <label className="emp-active-row">
          <span className="emp-active-text">
            ...
            <span className="emp-active-desc">{form.active ? 'Puede iniciar sesión' : 'No puede iniciar sesión'}</span>
          </span>
          <button
          type="button"
      ...
      <div className="emp-perms">
        <div className="emp-perms-head">
          ...
          <Chip tone={grantedCount === PERMISSIONS.length ? 'ok' : 'neutral'}>{grantedCount}/{PERMISSIONS.length}</Chip>
        </div>
      ...
      {initial &&
        <div className="emp-meta">
          <div className="emp-meta-item">
            ...
            <span className="emp-meta-v">{fmtLastActive(initial.lastActive)}</span>
          </div>
          <div className="emp-meta-item">
            <span className="emp-meta-k">Creado</span>
            <span className="emp-meta-v">{initial.createdAt ? (() => { const d = new Date(initial.createdAt); const m = d.toLocaleDateString('es', { month: 'long' }); return `${String(d.getDate()).padStart(2, '0')} ${m.charAt(0).toUpperCase() + m.slice(1)} ${d.getFullYear()}`; })() : '—'}</span>
          </div>
        </div>
      }
...
function EmployeeDetailSheet({ emp, onClose, onEdit, onDelete }: {
  emp: Employee | null;
  onClose: () => void;
  ...
  const fullAccess = emp.permissions === 'all' || permCount(emp) === PERMISSIONS.length;
  const roleLabel = ROLE_LABELS[emp.role] || emp.role;
  const perms = emp.permissions;
  const grantedPerms = perms === 'all' ? PERMISSIONS : PERMISSIONS.filter((p) => perms && perms[p.id]);
  const createdStr = emp.createdAt ? (() => { const d = new Date(emp.createdAt); const m = d.toLocaleDateString('es', { month: 'long' }); return `${String(d.getDate()).padStart(2, '0')} ${m.charAt(0).toUpperCase() + m.slice(1)} ${d.getFullYear()}`; })() : '—';

  return (
    <Sheet onClose={onClose} title="Detalle del empleado">
      <div className="prod-detail">
        <div className="prod-detail-head">
          <div className={`thumb emp-avatar ${isOwner ? 'owner' : ''} ${emp.role === 'admin' ? 'admin' : ''} ${emp.active === false ? 'inactive' : ''}`} style={{ width: 56, height: 56, fontSize: 18 }} aria-hidden="true">{initials(emp.name)}</div>
          <div style={{ minWidth: 0, flex: 1 }}>
            <div className="prod-detail-name">{emp.name}</div>
            <div style={{ display: 'flex', gap: 6, marginTop: 6, flexWrap: 'wrap' }}>
              <Chip tone={isOwner ? 'ok' : emp.role === 'admin' ? 'info' : 'neutral'}>{roleLabel}</Chip>
              {!isOwner && (emp.active === false ? <Chip tone="danger">Inactivo</Chip> : <Chip tone="ok">Activo</Chip>)}
            </div>
          </div>
        </div>
        ...
        <div className="emp-perms" style={{ marginTop: 4 }}>
          <div className="emp-perms-head">
            <span className="emp-perms-title">Permisos</span>
            <Chip tone={fullAccess ? 'ok' : 'neutral'}>{fullAccess ? 'Acceso total' : `${grantedPerms.length}/${PERMISSIONS.length}`}</Chip>
          </div>
          {grantedPerms.length === 0 ?
            <Banner tone="neutral" icon="lock" message="Este empleado no tiene permisos asignados." /> :
      ...
      <div className="prod-detail-actions">
        <Button icon="edit-3" onClick={() => onEdit(emp)} block>Editar</Button>
        <div className="row" style={{ gap: 10 }}>
          {!isOwner && <Button variant="danger" icon="trash-2" onClick={() => onDelete(emp)}>Eliminar</Button>}
...
// --- List row ----------------------------------------------------------------
function EmployeeRow({ emp, onEdit }: { emp: Employee; onEdit: (emp: Employee) => void }) {
  const roleLabel = ROLE_LABELS[emp.role] || emp.role;
  return (
    <div className="lrow emp-row" onClick={() => onEdit(emp)} style={{ cursor: 'pointer' }}>
      <div className={`thumb emp-avatar ${emp.role === 'owner' ? 'owner' : ''} ${emp.role === 'admin' ? 'admin' : ''} ${emp.active === false ? 'inactive' : ''}`} aria-hidden="true">{initials(emp.name)}</div>
      <div>
        <p className="pname">{emp.name}</p>
        <div className="pmeta emp-pmeta">
          <Chip tone={emp.role === 'owner' ? 'ok' : emp.role === 'admin' ? 'info' : 'neutral'}>{roleLabel}</Chip>
        </div>
      </div>
      <div className="pright emp-row-right">
        ...
        <span className="emp-permsum">{permCount(emp)} {permCount(emp) === 1 ? 'permiso' : 'permisos'}</span>)
        }
      </div>
...
export default function EmployeesScreen() {
  ...
  const employees = useQuery(api.employees.list, user ? { actorId: user._id } : 'skip') ?? [];
  ...
  filter((e) => status === 'all' ? true : status === 'active' ? e.active !== false : e.active === false).
  filter((e) => {
  ...
  filter((e) => {
    ...
    const t = norm(q);
    return norm(e.name).includes(t) || norm(e.email).includes(t) || norm(e.phone).includes(t);
  })
  ...
  const openEdit = (e: Employee) => {setDetail(null);setEditing(e);setEditorOpen(true);};
  ...
  const save = async (form: EmployeeFormValues) => {
    ...
    try {
      if (editing) {
        await updateEmployee({ actorId: user._id, employeeId: editing._id, patch: { ...form } });
      } else {
  ...
  return (
    <>
      <AppBar
        ...
        online={online}
        right={<Button size="sm" icon="user-plus" onClick={openNew}>Agregar empleado</Button>} />
      ...
      <div className="content stored-content" style={{ padding: '5px' }}>
        ...
        <div className="catalog-head" style={{ margin: '0 0 14px' }}>
          ...
          <div className="catalog-filters emp-filters">
            <label className="catalog-filter">
              <span>Rol</span>
              <select className="input cat-select" value={role} onChange={(e) => {setRole(e.target.value);setPage(1);}}>
            ...
            <label className="catalog-filter">
              <span>Estado</span>
              <select className="input cat-select" value={status} onChange={(e) => {setStatus(e.target.value);setPage(1);}}>
                <option value="all">Todos</option>
        ...
        <div className="hist-daterange-inline">
          <label className="catalog-filter">
            ...
            <DateField value={fromDate} max={toDate || new Date().toISOString().slice(0, 10)} onChange={(v) => {setFromDate(v);setPage(1);}} />
          </label>
          <label className="catalog-filter">
            <span>Creado hasta</span>
            <DateField value={toDate} min={fromDate || undefined} max={new Date().toISOString().slice(0, 10)} onChange={(v) => {setToDate(v);setPage(1);}} />
          </label>
        </div>
        ...
        <div className="card">
          ...
          <div className="empty" style={{ padding: '32px 16px' }}>
              <h4>Sin empleados</h4>
              <p>{q ? `Sin resultados para "${q}"` : 'Agrega tu primer cajero'}</p>
            </div> :
          visible.map((e) => <EmployeeRow key={e._id} emp={e} onEdit={openDetail} />)
          }
        </div>
        ...
        <div className="pager pager-sticky">
            ...
            <>Empleados <strong>{showingFrom}–{showingTo}</strong> de <strong>{list.length}</strong></>
            }
            </div>
