...
describe('Venta — ProductFoundSheet availability banner (no false "agotado")', () => {
  ...
  test('cart already holds all physical (stock 5, 5 in cart) → "máximo disponible en el carrito"; add disabled', () => {
    render(
      <ProductFoundSheet
  ...
  // Valid-max regression guard: empty cart → NO info banner at all (this is the
  // earlier false-"agotado" fix for the valid-max case).
  test('stock 30, empty cart → NO info banner (valid max, no false agotado)', () => {
    render(
      <ProductFoundSheet
        product={makeProduct({ stock: 30 })}
        currentCartQty={0}
  ...
  // The cart already holds some: "disponibles" = full physical 4 and the reason
  // is the cart. No en-espera clause is ever shown (reserved stock is removed).
  test('stock 4, 1 in cart → "4 disponibles" + "ya tienes 1 en el carrito" + "agregar 3 más"; no en-espera clause', () => {
    render(
      <ProductFoundSheet
