...
import type { CleanSplit, ClientSnapshot, MethodId, SaleItemSnapshot, SalesType, SplitRow } from '@/types';
...
export const METHOD_LABEL: Record<string, string> = Object.fromEntries(METHODS.map((m) => [m.id, m.label]));
export const METHOD_CURRENCY: Record<string, string> = Object.fromEntries(METHODS.map((m) => [m.id, m.currency]));
...
export const receiptMethod = (id: string) => RECEIPT_METHOD_LABEL[id] || METHOD_LABEL[id] || id;
...
export interface PaymentSheetProps {
  ...
  onConfirm: (method: string, tendered: number, splits: CleanSplit[]) => void | Promise<void>;
  salesType: SalesType;
  splits: SplitRow[];
...
export function PaymentSheet({ total, onClose, onConfirm, salesType: _salesType, splits, setSplits, nextIdRef, bsRate }: PaymentSheetProps) {
  ...
  const updateSplit = (id: number, patch: Partial<SplitRow>) => setSplits((s) => s.map((r) => r.id === id ? { ...r, ...patch } : r));
...
export function SuccessScreen({ invoice, total, items, method, splits, salesType, tendered, client, ivaPct, bsRate, onNew, onPrint, onDash: _onDash, online }: SuccessScreenProps) {
  ...
  const bsNum = (usd: number) => (usd * rate).toLocaleString('es-VE', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
          ...
          <div className="card receipt">
            ...
            {salesType === 'invoice'
              ? <>
                  ...
                  <div className="rcpt-kv"><span>IVA ({ivaPct}%)</span><span>Bs {bsNum(tax)}</span></div>
                </>
