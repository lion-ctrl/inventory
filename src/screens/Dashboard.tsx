...
import { useBsRate, useCategories, useClients, useProducts } from '@/state/hooks';
...
export default function Dashboard() {
  ...
  const todaySales = salesHistory.filter((s) => sameDay(new Date(s.soldAt), now) && notRefunded(s));
  const yesterSales = salesHistory.filter((s) => sameDay(new Date(s.soldAt), yesterday) && notRefunded(s));
  ...
  const recent = [...salesHistory].sort((a, b) => b.soldAt - a.soldAt).slice(0, 5);

  const METHOD_LABEL: Record<string, string> = { cash: 'Efectivo', cash_bs: 'Efectivo Bs', card: 'Tarjeta', transfer: 'Transferencia', mobile: 'Pago móvil', zelle: 'Zelle' };
  const timeOf = (ms: number) => new Date(ms).toLocaleTimeString('es', { hour: '2-digit', minute: '2-digit' });
            ...
            <div className="card">
              ...
              <div className="lrow" key={p._id}>
                  ...
                  <div>
                    ...
                    <div className="pmeta mono">{p.sku} · {catById.get(p.categoryId) || ''}</div>
                  </div>
          ...
          <section className="dash-col">
            ...
            <div className="card">
              ...
              <div className="lrow" key={s._id}>
                  <div className={`thumb dash-sale-thumb ${s.refund ? 'refunded' : ''}`}>
                    <Icon name={s.refund ? 'rotate-ccw' : 'receipt'} size={18} />
                  </div>
                  <div>
                    <p className="pname mono" style={{ fontWeight: 600 }}>N° {s.invoiceNumber}</p>
                    <div className="pmeta">{s.client?.name || 'Sin cliente'} · {METHOD_LABEL[s.method] || s.method} · {timeOf(s.soldAt)}</div>
                  </div>
                  <div className="pright">
                    <div className={`dash-sale-amt ${s.refund ? 'refunded' : ''}`}>${s.total.toFixed(2)}</div>
                    {s.refund && <div className="dash-sale-tag">Reembolsada</div>}
                  </div>
                </div>
              )}
            </div>
