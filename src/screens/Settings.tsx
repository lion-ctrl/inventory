...
function SettingRow({ icon, label, value, onClick, last, danger }: SettingRowProps) {
  return (
...
function SettingToggleRow({ icon, label, sub, value, onChange, last }: SettingToggleRowProps) {
  return (
...
function SettingsEditSheet({ title, fields, values, onSave, onClose }: SettingsEditSheetProps) {
...
export default function SettingsScreen() {
  ...
  const CURRENCY_LABEL: Record<string, string> = { USD: 'US$ Dólar', PAB: 'B/. Balboa', EUR: '€ Euro', MXN: '$ Peso MX', COP: '$ Peso CO' };
    ...
    <>
      ...
      <div className="content set-content">
        ...
        <div className="set-account">
          ...
          <div className="set-account-info">
            ...
            <div className="set-account-role">
              <Chip tone="ok">{(user?.role && ROLE_LABELS[user.role]) || user?.role || 'Cajero'}</Chip>
        ...
        <SettingsSection title="Tienda" hint="Datos para facturas y recibos">
          <SettingRow icon="store" label="Nombre" value={storeName} onClick={() => setEditor({ kind: 'store' })} />
          <SettingRow icon="hash" label="RIF" value={storeRif} onClick={() => setEditor({ kind: 'store' })} />
          <SettingRow icon="phone" label="Teléfono" value={phone} onClick={() => setEditor({ kind: 'store' })} />
          <SettingRow icon="map-pin" label="Dirección" value={address} onClick={() => setEditor({ kind: 'store' })} last />
        </SettingsSection>

        {/* Billing & taxes */}
        <SettingsSection title="Facturación e impuestos">
          <SettingRow icon="percent" label={`Impuesto (${taxName})`} value={`${ivaPct}%`} onClick={() => setEditor({ kind: 'billing' })} />
        ...
        <SettingsSection title="Sincronización" hint="Conexión con el servidor">
          <div className="set-sync-row">
            ...
            <span className="set-row-text">
              ...
              <span className="set-row-sub">{online ? 'Sincronización en tiempo real' : 'Trabajando sin conexión'}</span>
            </span>
