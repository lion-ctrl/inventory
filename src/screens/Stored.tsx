...
import { AppBar, Button, ConfirmDialog, Icon, IconButton, Input, Sheet } from '@/components';
...
export function DateField({ value, min, max, onChange }: {
  value: string;
  min?: string;
  max?: string;
...
export default function StoredCartsScreen() {
  ...
  const taxOf = (c?: HeldCart['client']) => c?.taxId ? (c.taxPrefix ? `${c.taxPrefix}-${c.taxId}` : c.taxId) : '';
    ...
    <>
      ...
      <div className="content stored-content" style={{ padding: "5px", textAlign: "left" }}>
        ...
        <div className="stored-grid">
            ...
            const paidSoFar = (s.splits || []).reduce((acc, r) => acc + splitUsd(r, bsRate), 0);
      ...
      {detailCart &&
      <Sheet onClose={() => setDetailCart(null)} title={`Productos · ${detailCart.code}`}>
