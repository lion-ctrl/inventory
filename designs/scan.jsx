...
function ProductInfoSheet({ product, onClose, bsRate }) {
  const p = product;
      ...
      <div className="scan-card scan-card-insheet">
        <div className="scan-card-head">
          ...
          <div className="scan-card-id">
            ...
            <div className="scan-card-cat">{p.cat}</div>
          </div>
        </div>
        ...
        <dl className="scan-card-meta">
          ...
          <div><dt>Categoría</dt><dd>{p.cat}</dd></div>
          <div><dt>IVA</dt><dd>{p.exempt === true ? 'Exento' : '13%'}</dd></div>
        </dl>
      </div>
...
function ProductMissSheet({ code, onClose }) {
  return (
...
function ScanScreen({ products, online, onBack, bsRate, tweaks = {} }) {
  const catalog = React.useMemo(
    () => (products && products.length ? products : CATALOG),
    [products]);

  const [selected, setSelected] = React.useState(null);   // product to show
  const [missCode, setMissCode] = React.useState(null);   // not-found code
  const [query, setQuery] = React.useState('');
  const [activeCat, setActiveCat] = React.useState('all');
  const [sort, setSort] = React.useState('name-asc');

  const handleScanResult = React.useCallback((r) => {
    if (r.found) setSelected(r.product);
    else setMissCode(r.code);
  }, []);

  const filtered = (activeCat === 'all' ? catalog : catalog.filter(p => p.cat === activeCat))
    .filter(p => {
  ...
  return (
    <>
      ...
      <div className="content scan-screen">
        ...
        <div className="pos-scanner-sticky scan-scanner">
          <ScannerView
            ...
            mode="split"
            density="roomy"
            demoControls={tweaks.demoControls}
            catalog={catalog} />
        </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="quick-grid scan-grid">
            {filtered.map(p =>
              <button className="quick-tile" key={p.id} onClick={() => setSelected(p)}>
      ...
      {selected && <ProductInfoSheet product={selected} bsRate={bsRate} onClose={() => setSelected(null)} />}
