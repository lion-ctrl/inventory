// Employees / Team screen — owner-only RBAC management of cashiers.
// CRUD over EMPLOYEES with per-cashier permission toggles + active status.
...
function permCount(emp) {
  if (emp.permissions === 'all') return PERMISSIONS.length;
  return PERMISSIONS.filter((p) => emp.permissions && emp.permissions[p.id]).length;
}
...
function EmployeeForm({ initial, onSave, onDelete, onCancel }) {
  ...
  const togglePerm = (id, v) => setForm((f) => ({ ...f, permissions: { ...f.permissions, [id]: v } }));
  ...
  const initialPerms = initial?.permissions === 'all'
    ? Object.fromEntries(PERMISSIONS.map((p) => [p.id, true]))
    : (initial?.permissions || {});
...
function EmployeeDetailSheet({ emp, onClose, onEdit, onDelete }) {
  ...
  const grantedPerms = emp.permissions === 'all' ? PERMISSIONS : PERMISSIONS.filter((p) => emp.permissions && emp.permissions[p.id]);
...
// --- Main screen -------------------------------------------------------------
function EmployeesScreen({ employees, setEmployees, currentUserId, online, onBack }) {
  ...
  const openEdit = (e) => {setDetail(null);setEditing(e);setEditorOpen(true);};
  ...
  const save = (form) => {
    if (editing) {
      ...
      const id = 'E-' + String(1000 + Math.floor(Math.random() * 9000));
      setEmployees((prev) => [...prev, { id, ...form, createdAt: new Date().toISOString().slice(0, 10), lastActive: null }]);
    }
    setEditorOpen(false);
  };
    ...
    <>
      ...
      <div className="content stored-content" style={{ padding: '5px' }}>
        ...
        <div className="card">
          ...
          visible.map((e) => <EmployeeRow key={e.id} emp={e} onEdit={openDetail} />)
          }
        </div>
...
window.EmployeeDetailSheet = EmployeeDetailSheet;