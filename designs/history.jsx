...
const METHOD_LABELS = { cash: 'Efectivo dólar', cash_bs: 'Efectivo Bs', card: 'Tarjeta', transfer: 'Transferencia', mobile: 'Pago móvil', zelle: 'Zelle' };
const METHOD_ICONS = { cash: 'banknote', cash_bs: 'banknote', card: 'credit-card', transfer: 'arrow-left-right', mobile: 'smartphone', zelle: 'dollar-sign' };
const METHOD_TONES = { cash: 'ok', cash_bs: 'ok', card: 'info', transfer: 'neutral', mobile: 'purple', zelle: 'ok' };
...
function inRange(sale, range, fromDate, toDate) {
  ...
  switch (range) {
    ...
    case 'week':return now.getTime() - d.getTime() <= 7 * 86400000;
...
function DateInputDMY({ value, min, max, onChange, placeholder = 'dd/mm/aaaa' }) {
  const [text, setText] = React.useState(value ? fmtDMY(value) : '');
  React.useEffect(() => {setText(value ? fmtDMY(value) : '');}, [value]);

  const commit = (raw) => {
    const digits = raw.replace(/\D/g, '').slice(0, 8);
    let out = digits;
    if (digits.length > 4) out = `${digits.slice(0, 2)}/${digits.slice(2, 4)}/${digits.slice(4)}`;else
...
function formatDateTime(iso) {
  ...
  const t = d.toLocaleTimeString('es', { hour: 'numeric', minute: '2-digit', hour12: true });
...
function SaleTableRow({ sale, onClick, bsRate }) {
  const clientGlyph = sale.client?.kind === 'business' ? '🏢' : (sale.client?.name?.[0] || '?').toUpperCase();
  const itemCount = sale.items.reduce((s, i) => s + i.qty, 0);
  const isMixto = sale.splits && sale.splits.length > 1;
  const isRefunded = sale.status === 'refunded';
  return (
    <div className={`hist-row ${isRefunded ? 'is-refunded' : ''}`} onClick={() => onClick(sale)}>
      <div className="hist-col-id">
        <div className="hist-cell-id mono">N° {sale.id}{isRefunded && <span className="hist-refund-tag">Reembolsada</span>}</div>
        <div className="hist-cell-time">{formatDateTime(sale.date)}</div>
        {isRefunded && sale.refundedAt &&
        <div className="hist-cell-refund">Reemb. {formatDateTime(sale.refundedAt)}</div>}
      </div>
      <div className="hist-col-client">
        <div className="hist-avatar">{clientGlyph}</div>
        <div style={{ minWidth: 0, flex: 1 }}>
          <div className="hist-client-name">{sale.client?.name || 'Sin cliente'}</div>
          <div className="hist-client-tax mono">
            {sale.client?.taxId ? `${sale.client.taxPrefix || ''}-${sale.client.taxId}` : '—'}
          </div>
        </div>
      </div>
      ...
      <div className="hist-col-method">
        <Chip tone={isMixto ? 'info' : METHOD_TONES[sale.method] || 'neutral'}>
          <Icon name={isMixto ? 'shuffle' : METHOD_ICONS[sale.method] || 'tag'} size={12} />
          {isMixto ? 'Mixto' : METHOD_LABELS[sale.method]}
        </Chip>
      </div>
      <div className="hist-col-total">
        <span className="cur">$</span><span className="amt tabular">{sale.total.toFixed(2)}</span>
        {bsRate > 0 && <span className="hist-total-bs tabular">Bs {(sale.total * bsRate).toFixed(2)}</span>}
      </div>
      <div className="hist-col-chevron"><Icon name="chevron-right" size={18} /></div>
    </div>);

}

function SaleRow({ sale, onClick, bsRate }) {
  const clientGlyph = sale.client?.kind === 'business' ? '🏢' : (sale.client?.name?.[0] || '?').toUpperCase();
  const itemCount = sale.items.reduce((s, i) => s + i.qty, 0);
  const isMixto = sale.splits && sale.splits.length > 1;
  const isRefunded = sale.status === 'refunded';
  return (
    <div className={`stored-card hist-card ${isRefunded ? 'is-refunded' : ''}`} onClick={() => onClick(sale)}>
      <div className="stored-card-head">
        <div className="stored-card-id mono">N° {sale.id}</div>
        {isRefunded ?
        <span className="hist-refund-tag">Reembolsada</span> :
        <div className="stored-card-time">{formatDateTime(sale.date)}</div>}
      </div>
      {isRefunded &&
      <div className="hist-card-dates">
        <span>Venta: {formatDateTime(sale.date)}</span>
        {sale.refundedAt && <span>Reemb.: {formatDateTime(sale.refundedAt)}</span>}
      </div>}
      <div className="stored-card-client">
        <div className="stored-card-avatar">{clientGlyph}</div>
        <div style={{ minWidth: 0, flex: 1 }}>
          <div className="stored-card-name">{sale.client?.name || 'Sin cliente'}</div>
          {sale.client?.taxId &&
      ...
      <div className="stored-card-summary">
        ...
        <div className="stored-card-cell">
          ...
          {bsRate > 0 && <span className="v-bs tabular">Bs ${(sale.total * bsRate).toFixed(2)}</span>}
        </div>
        <div className="stored-card-cell">
...
function SaleDetailSheet({ sale, onClose, onReprint, onRefund, canRefund, online, bsRate }) {
  if (!sale) return null;
  ...
  const isRefunded = sale.status === 'refunded';
  const refundedStr = sale.refundedAt ? formatDateTime(sale.refundedAt) : '';
  return (
    <Sheet onClose={onClose} title={`${docPrefix} N° ${sale.id}`}>
      <div className="hist-detail">
        ...
        <div className="hist-refunded-banner">
            ...
            <div>
              ...
              <div className="hist-refunded-sub">
                {refundedStr}{sale.refundedBy ? ` · por ${sale.refundedBy}` : ''}
        ...
        <div className="hist-detail-client">
          <div className="hist-avatar lg">
            {sale.client?.kind === 'business' ? '🏢' : (sale.client?.name?.[0] || '?').toUpperCase()}
          </div>
          <div style={{ minWidth: 0, flex: 1 }}>
            <div className="hist-detail-cname">{sale.client?.name || 'Sin cliente'}</div>
            {taxDisplay && <div className="hist-detail-ctax mono">{taxDisplay}</div>}
          </div>
        </div>
        ...
        <div className="hist-detail-cashier">
          ...
          <span className="v">{sale.cashier}</span>
        </div>

        <div className="hist-detail-section-title">Productos ({sale.items.reduce((s, i) => s + i.qty, 0)})</div>
        <div className="hist-detail-items">
          ...
          <div className="hist-item-row" key={i}>
              ...
              <div className="hist-item-info">
                ...
                <div className="hist-item-sku mono">{it.sku} · {it.barcode}</div>
                <div className="hist-item-sku mono">${it.price.toFixed(2)} c/u × {it.qty}</div>
              </div>
              <div className="hist-item-right">
                {it.cat && <Chip tone="neutral" style={{ textTransform: 'capitalize' }}>{it.cat}</Chip>}
                <div className="hist-item-total tabular">${(it.price * it.qty).toFixed(2)}</div>
                {bsRate > 0 && <div className="hist-item-total-bs tabular">Bs ${(it.price * it.qty * bsRate).toFixed(2)}</div>}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="hist-detail-totals">
          <div className="line"><span>Subtotal</span><span className="hist-line-amt"><span className="tabular">${sale.subtotal.toFixed(2)}</span>{bsRate > 0 && <span className="hist-line-bs tabular">Bs {(sale.subtotal * bsRate).toFixed(2)}</span>}</span></div>
          {sale.tax > 0 &&
          <div className="line"><span>IVA (13%)</span><span className="hist-line-amt"><span className="tabular">${sale.tax.toFixed(2)}</span>{bsRate > 0 && <span className="hist-line-bs tabular">Bs {(sale.tax * bsRate).toFixed(2)}</span>}</span></div>
          }
          <div className="line total"><span>Total</span><span>${sale.total.toFixed(2)}</span></div>
          {bsRate > 0 && <div className="line total-bs"><span>Total Bs</span><span className="tabular">Bs ${(sale.total * bsRate).toFixed(2)}</span></div>}
        </div>
        ...
        <div className="hist-detail-payment">
          ...
          <div className="hist-pay-list">
            ...
            <div className="hist-pay-row" key={i}>
                  ...
                  <span className="hist-pay-amt"><span className="tabular">${s.amount.toFixed(2)}</span>{bsRate > 0 && <span className="hist-pay-bs tabular">Bs ${(s.amount * bsRate).toFixed(2)}</span>}</span>
                </div>
            )}
            </div> :
          ...
          <div className="hist-pay-row">
              ...
              <span className="hist-pay-amt"><span className="tabular">${sale.total.toFixed(2)}</span>{bsRate > 0 && <span className="hist-pay-bs tabular">Bs ${(sale.total * bsRate).toFixed(2)}</span>}</span>
            </div>
          }
        </div>
      </div>
      ...
      <div className="hist-detail-actions hist-detail-actions-col">
        ...
        <Button variant="danger" icon="rotate-ccw" block onClick={() => onRefund(sale)}>Reembolsar</Button>}
        <div className="row" style={{ gap: 10 }}>
          <Button variant="secondary" onClick={onClose}>Cerrar</Button>
          <Button icon="printer" onClick={() => onReprint && onReprint(sale)}>Reimprimir</Button>
        </div>
      </div>
    </Sheet>);
...
function RefundSheet({ sale, onConfirm, onCancel, bsRate }) {
  const REASONS = ['Producto defectuoso', 'Error en el cobro', 'Cliente desistió', 'Producto equivocado', 'Otro'];
      ...
      <div className="refund-sheet">
        <div className="refund-summary">
          <div className="refund-summary-row">
            ...
            <span className="v mono">N° {sale.id}</span>
          </div>
          <div className="refund-summary-row">
          ...
          <div className="refund-summary-row total">
            ...
            <span className="v tabular">${sale.total.toFixed(2)}{bsRate > 0 ? ` · Bs ${(sale.total * bsRate).toFixed(2)}` : ''}</span>
          </div>
        </div>

        <div className="refund-field">
          <span className="refund-label">Motivo del reembolso<span className="req"> *</span></span>
        ...
        <label className="refund-field">
            <span className="refund-label">Especifica el motivo<span className="req"> *</span></span>
            <textarea
            className="input client-textarea"
            rows={2}
            value={note}
            onChange={(e) => setNote(e.target.value)}
            placeholder="Describe el motivo del reembolso" style={{ padding: "5px 14px" }} />
          </label>
        }

        <Banner tone="warn" icon="alert-triangle" message="El reembolso devuelve las unidades al inventario y marca la venta como reembolsada. Esta acción no se puede deshacer." />
      </div>

      <div className="hist-detail-actions">
        <Button variant="secondary" onClick={onCancel}>Cancelar</Button>
        <Button variant="danger" icon="rotate-ccw" disabled={!canConfirm} onClick={() => onConfirm(sale, finalReason)}>
          Confirmar reembolso
        </Button>
      </div>
    </Sheet>);
...
function HistoryScreen({ sales, setSales, user, online, onBack, bsRate, onRestock }) {
  ...
  const canRefund = can(user, 'void_sales');

  const doRefund = (sale, reason) => {
    // Return the sold units back to inventory.
    if (onRestock && sale.items) onRestock(sale.items.map((i) => ({ id: i.id, qty: i.qty })));
    setSales((prev) => prev.map((s) => s.id === sale.id ? {
      ...s,
      status: 'refunded',
      refundedAt: new Date().toISOString(),
      refundedBy: user?.name || 'Sistema',
      refundReason: reason
    } : s));
    setRefunding(null);
    setDetail(null);
  ...
  filter((s) => {
    ...
    if (statusFilter === 'refunded') return s.status === 'refunded';
    return s.status !== 'refunded'; // 'completed'
  }).
  filter((s) => {
    const term = norm(q);
    if (!term) return true;
    return norm(s.id).includes(term) || norm(s.client?.name).includes(term) || norm(s.client?.taxId).includes(term);
  });
  ...
  const stats = React.useMemo(() => {
    const active = filtered.filter((x) => x.status !== 'refunded');
    const count = active.length;
    const revenue = active.reduce((s, x) => s + x.total, 0);
    const avg = count > 0 ? revenue / count : 0;
    const units = active.reduce((s, x) => s + x.items.reduce((u, i) => u + i.qty, 0), 0);
  ...
  return (
    <>
      ...
      <div className="content hist-content">
        ...
        <div className="prod-stats hist-stats">
          ...
          <div className="prod-stat">
            ...
            <span className="meta">{bsRate > 0 ? `Bs ${(stats.revenue * bsRate).toFixed(2)}` : 'total facturado'}</span>
          </div>
          <div className="prod-stat">
            <span className="k">Productos</span>
        ...
        <div className="hist-daterange-inline">
          <label className="catalog-filter">
            <span>Desde</span>
            <window.DateField value={fromDate} max={toDate || TODAY_ISO} onChange={(v) => {setFromDate(v);setRange('custom');resetPage();}} />
          </label>
          <label className="catalog-filter">
            <span>Hasta</span>
            <window.DateField value={toDate} min={fromDate || undefined} max={TODAY_ISO} onChange={(v) => {setToDate(v);setRange('custom');resetPage();}} />
          </label>
        </div>
        ...
        <div className="catalog-head" style={{ margin: '0 0 12px' }}>
          ...
          <div className="catalog-filters hist-filters">
            <label className="catalog-filter">
              <span>Método</span>
              <select className="input cat-select" value={methodFilter} onChange={(e) => {setMethodFilter(e.target.value);resetPage();}}>
            ...
            <label className="catalog-filter">
              <span>Estado</span>
              <select className="input cat-select" value={statusFilter} onChange={(e) => {setStatusFilter(e.target.value);resetPage();}}>
        ...
        <div className="hist-table">
            ...
            {visible.map((s) => <SaleTableRow key={s.id} sale={s} onClick={setDetail} bsRate={bsRate} />)}
          </div>
        }
        {sorted.length > 0 &&
        <div className="stored-grid hist-cards">
            {visible.map((s) => <SaleRow key={s.id} sale={s} onClick={setDetail} bsRate={bsRate} />)}
          </div>
        }
      ...
      <SaleDetailSheet
        ...
        onReprint={(s) => alert(`Reimprimir ${s.id} (demo)`)} />
      }
...
window.DateInputDMY = DateInputDMY;